...
tourism_day_visits_location_clean %>% 
  ...
  coord_flip()
```
 _Answer_ Correlation between area visited and millions pounds expenditure for 2019
 From 2013 to 2019
...
labs(
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
_Answer_ this visualization see the evolution for the period 2013 - 2019 the visits in city/ large town has fallen but the expenditure there has increase in the last year. the Rural countryside shows the opposite the expenditure has decrease but  the visits has increase from 2018.
...
# tourism_day_visits_location_clean + tourism_day_visits_activities_clean = answer question2!!!!!! THere are no enough information to combine 'tourism activity' and 'type of location visited'. Has to be the visualizations in Q2!!
...
visit_sites_activity %>%
  filter(breakdown_of_domestic_tourism == 'Visits') %>% 
  filter(type_of_location_visited != 'All areas') %>% 
  filter(tourism_activity != c('All', 'NA')) %>% 
  #filter(year ==2019) %>% 
  arrange(year, tourism_activity) %>% 
ggplot(aes( x = type_of_location_visited, y = value))+
  geom_col(fill = "light blue")+
labs(
    title = "GB visits by location  in Scotland 2019",
    x = "\n",
    y = "millions Pounds\n")
...
```
Total value of All "types_of_location_visited" (124) is different to the independent sum of the each value, check what they say in the web. But I think the answer is that one person can visited multiple location. It is the same even you dont filter NA.  
...
_Extension_
Decission Tree Age-expenditure. Didnt work NO Enough Data
