...
```
 "S"pending 3 or more hours out of the home"
...
tourism_day_visits_location_clean %>% 
  ...
  coord_flip()
```
 -Answer_ Correlation between area visited and millions pounds expenditure for 2019
 From 2013 to 2019
...
labs(
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
_Answer_ this voisualization see the evolution for the period 2013 - 2019 the visits in city/ large town has fallen but the expenditure there has increase in the last year. the Rural countryside shows the opposite the expenditure has decrease but  the visits has increase from 2018.
...
It can be observed   that for the year 2019  visits and expenditure are correlated except for the values Village/Rural countryside. In happens as well in the year 2018, 2014
 ...
 Q2 _Which kind of activity generates the most visits to sites?_ /// SECOND PART- REPETITION
...
summarise?? para que salgan juntos todos los anos.
...
labs(
    ...
    scale_colour_brewer(guide = "none", palette = "Set2")
```
_Answer_ Spend more Female than Male in 2019
```{r}
tourism_day_visits_demmographics_clean %>% 
  #filter(year == 2019) %>% 
  filter(breakdown_of_domestic_tourism =='Expenditure') %>% 
...
labs(
    ...
    scale_x_continuous(breaks = 2013:2019)
```
_Answer_ From records always spend more female and male
# Marital Status
...
labs(
    ...
    scale_colour_brewer(guide = "none", palette = "Set2")
```
_Answer_ Spend more without Kids in 2019
```{r}
tourism_day_visits_demmographics_clean %>% 
  #filter(year == 2019) %>% 
  filter(breakdown_of_domestic_tourism =='Expenditure') %>% 
...
labs(
    ...
    scale_x_continuous(breaks = 2013:2019)
```
_Answer_ Spend more without Kids in all years
...
labs(
    ...
    scale_colour_brewer(guide = "none", palette = "Set2")
```
_Answer_ for 2019 is obvious that the weitgh of spend is in the working group, but the other 2 groups means 500 millions of pounds, is a lot money?
...
tourism_day_visits_demmographics_clean %>% 
 ...
 ggplot(aes( x = social_grade, y = value ))+
  geom_col(fill = "light blue")+
labs(
    title = "GB Expenditure By Socil Grade in Scotland 2019",
    x = "\n Social Grade",
    y = "millions Pounds\n") +
    scale_colour_brewer(guide = "none", palette = "Set2")
```
...
tourism_day_visits_demmographics_clean %>% 
 ...
 ggplot(aes( x = year, y = value ))+
  ...
  labs(
    ...
    scale_x_continuous(breaks = 2013:2019)
```
The obvious for social grade but not the last year. Is it ethical?


_Answer_. so Young couples, maybe more females  with good salaries/ highest employment status, high social grade and no children (yet)
...
There are few things interesting, 0.5 million visitis arrive by see(boat/ship/ ferry) where is cruise is boat a cruise? ship and ferry can include go to the Islands? only 0.4 take a caravan or campervan but the value of their spend is 24.2 millions of pounds. We can conclude that is minimum visits some Islands such as Mull, Islay, Arran, others such as western Islands , Orkney or Shetlands have airports. Visit  the Islands imply more nights and more expenses
...
predict_spend %>% 
  ...
  labs(
    title = "GB  Expenditure in Scotland between 2011 -2019",
    x = "\nYear",
    y = "millions Pounds\n") +
   scale_x_continuous(breaks = 2013:2019)
