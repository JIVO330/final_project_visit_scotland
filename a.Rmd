...
The first question should it should be ask is 'how much money means the domestic tourist activity for Scotland?'.
...
The higher number of higher visits for the same year was...
...
Even if the large cities attract the majority of visits and expenditure, there is a - relative- decrease in comparison with other locations such as countryside or seaside.
```{r}
tourism_day_visits_location_clean %>% 
  select(year, value, type_of_location_visited, breakdown_of_domestic_tourism) %>% 
  filter(breakdown_of_domestic_tourism != 'Expenditure') %>% 
...
This visualization shows the evolution for the period 2013 - 2019 the visits in city/ large town has fallen but the expenditure there has increase in the last year. the Rural countryside shows the opposite the expenditure has decrease but  the visits has increase from 2018. It can be observed  that for the year 2019  visits and expenditure are correlated except for the values Village/Rural countryside. In happens as well in the year 2018 and 2014.
...
## Understanding the visitors from 2013  till 2019 {.smaller}

This is  the general evolution of visiting Scotland by age. It can argue that there is not a clear tendency, each group of age had a preferred year in visit Scotland
...
## And previous years
It can be appreciated that looks like a normal tendency where female expend more money than male
```{r}
tourism_day_visits_demmographics_clean %>% 
  #filter(year == 2019) %>% 
  filter(breakdown_of_domestic_tourism =='Expenditure') %>% 
...
## Marital Status in 2019
 The level of expenditure is higher in married than not married.
...
The next two plots show something obvious,for 2019 is obvious that the weigh of spend is in the working group, but the other 2 groups means 500 millions of pounds, is a lot money?
...
Is important the social grade? Is ethical? Funny the most expenditure is the highest grade occupation but the second is in theory the lowest grade occupation ( less money/ less cultured person? assumption, another ethical consideration)
```{r}
tourism_day_visits_demmographics_clean %>% 
...
- Only 0.4 take a caravan or campervan but the value of their spend is 24.2 millions of pounds. 
...
## Where they paying out
  ...
  The data confirms that they are the large cities (Edinburgh and Glasgow) and the Highlands the places the centers of the expenditure. Places in relation with the use of car and train as a main transport.
```{r}
...
## Go to...
For the plot, it could be deduced that is not only  how to arrive,it is the offer of activities. An example could be  the fact that The Western Islands ( Na h-Elleanan Siar) is over regions of the 'Central bell'.
